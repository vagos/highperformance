...
double find_knn_value(double *p, int n, int knn, kdt::KDTree<Point>& tree)
{
	int nn_x[MAX_NNB];
	double nn_d[MAX_NNB];

	compute_knn_brute_force(xdata, p, TRAINELEMS, PROBDIM, knn, nn_x, nn_d); // brute-force /linear search
	// compute_knn_kdtree(xdata, p, TRAINELEMS, PROBDIM, knn, nn_x, nn_d, tree);
...
int main(int argc, char *argv[])
{
    ...
    std::vector<Point> points(TRAINELEMS);
...
#endif
		t0 = gettime();
		double yp = find_knn_value(x, PROBDIM, NNBS, tree);
		t1 = gettime();
		t_sum += (t1-t0);
