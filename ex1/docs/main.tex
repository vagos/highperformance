...
\section{Chekpointing με MPI I/O}
    ...
    non-ghost cells πρώτα στον ενδιάμεσο buffer \src{rho\_tmp} πριν τον κάνουμε
    serialize τελικά στο αρχείο.
        ...
        \subsubsection*{\src{write\_density\_mpi()}}
            \begin{itemize}
                ...
                \item Αποθήκευση του πίνακα \src{rho\_tmp} στο αρχείο χρησιμοποιώοντας την blocking κλήση κλήση \src{MPI\_File\_write}
            \end{itemize}
        \subsubsection*{\src{read\_density\_mpi()}}
            \begin{itemize}
                \item Άνοιγμα αρχείου για \src{read}
                \item Υπολογισμός \src{length} και \src{offset}
                \item Δίαβασμα \src{len doubles} από \src{offset} και αποθήκευση στο \src{rho\_tmp} χρησιμοποιώντας την blocking κλήση κλήση \src{MPI\_File\_read}.
