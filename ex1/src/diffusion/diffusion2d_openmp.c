...
void write_step_data(Diffusion2D *D2D, char* filename)
{
    FILE* step_file = fopen(filename, "w+");

    for (int i = 1; i <= D2D->N_; ++i) {
    for (int j = 1; j <= D2D->N_; ++j) {
        fprintf(step_file, "%f\n", D2D->rho_[i*D2D->real_N_ + j]);
    }
    }
}
...
#pragma omp master
    ...
    if (step % 1000 == 0)  {
        ...
        sprintf(step_filename, "plots/data/density_step_%d.dat", step);
        write_step_data(&system, step_filename);
    }
#endif
